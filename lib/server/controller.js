	...
	return function(req, res, next) {
		...
		if (proxy.shouldProxy(req)) {
			if (req.appium.commandTimeout) {
				...
				// so let's make sure to reset the timeout on every request
				// still
...
exports.sessionBeforeFilter = function(req, res, next) {
	...
	if (sessId && (!req.device || req.appium.sessionId !== sessId)) {
		safely(req, function() {
			res.status(404).send({
				sessionId : null,
				status : status.codes.NoSuchDriver.code,
				value : ''
			});
		});
	} else {
...
exports.installApp = function(req, res) {
	...
	if (typeof req.body.appPath !== "undefined") {
						...
						function(unpackedAppPath) {
							if (unpackedAppPath === null) {
								respondError(req, res,
										'Only a (zipped) app/apk files can be installed using this endpoint');
							} else {
		...
		respondError(req, res,
				"No app defined (either through desired capabilities or as an argument)");
	}
};

exports.removeApp = function(req, res) {
	req.body.appId = req.body.appId || req.body.bundleId;
	if (checkMissingParams(req, res, {
				...
				.isAppInstalled(
						...
						function(error, stdout) {
							if (error !== null) {
								...
								// We're examining the type of stdout because
								// `isAppInstalled` uses
								// node-idevice for real iOS devices.
								// node-idevice passes a boolean
								// value in the second parameter of the callback
								// function. Other
								// functions pass back an array. Changing the
								// parameter type of the
								// other functions is a deeper and more
								// dangerous change than just
								// type-checking here.
...
exports.closeApp = function(req, res) {
	req.device.stop(function() {
		respondSuccess(req, res, "Successfully closed the ["
				+ req.device.args.app + "] app.");
	}, function() {
		respondError(req, res, "Something went wrong whilst closing the ["
				+ req.device.args.app + "] app.");
	});
};
...
exports.createSession = function(req, res) {
	...
	var next = function(reqHost, sessionId) {
		safely(req, function() {
			res.set('Location', "http://" + reqHost + "/wd/hub/session/"
					+ sessionId);
			res.status(303).send(
					"Appium session started with sessionId " + sessionId);
		});
	};
	if (req.appium.preLaunched && req.appium.sessionId) {
		...
		req.appium.start(req.body.desiredCapabilities, function(err, instance) {
			if (err) {
				logger.error("Failed to start an Appium session, err was: "
						+ err);
				logger.debug(err.stack);
				delete err.stack;
				respondError(req, res, status.codes.SessionNotCreatedException,
						err);
			} else {
				logger.debug("Appium session started with sessionId "
						+ req.appium.sessionId);
...
exports.equalsElement = function(req, res) {
	var element = req.params.elementId, other = req.params.otherId;

	req.device.equalsWebElement(element, other, getResponseHandler(req, res));
};

exports.findElements = function(req, res) {
	var strategy = req.body.using, selector = req.body.value;

	if (checkMissingParams(req, res, {
		strategy : strategy,
		selector : selector
	}, true)) {
		req.device.findElements(strategy, selector,
...
exports.findElement = function(req, res) {
	...
	if (checkMissingParams(req, res, {
		...
		req.device
				.findElement(strategy, selector, getResponseHandler(req, res));
	}
};

exports.findElementFromElement = function(req, res) {
	var element = req.params.elementId, strategy = req.body.using, selector = req.body.value;

	req.device.findElementFromElement(element, strategy, selector,
			getResponseHandler(req, res));
};
...
exports.findElementsFromElement = function(req, res) {
	...
	req.device.findElementsFromElement(element, strategy, selector,
			getResponseHandler(req, res));
};
...
exports.performTouch = function(req, res) {
	...
	if (gestures.length === 4 && gestures[0].action === 'press'
			&& gestures[1].action === 'wait' && gestures[2].action === 'moveTo'
...
exports.performMultiAction = function(req, res) {
	...
	if (actions.length === 0) {
		return respondError(
				...
				new Error(
						"Unable to perform Multi Pointer Gesture with no actions."));
	}

	req.device.performMultiAction(elementId, actions, getResponseHandler(req,
			res));
};
...
exports.touchLongClick = function(req, res) {
	...
	if (element && checkMissingParams(req, res, {
		element : element
	}, true)) {
		req.device.touchLongClick(element, x, y, duration, getResponseHandler(
				req, res));
	} else if (checkMissingParams(req, res, {
		x : x,
		y : y
	}, true)) {
		req.device.touchLongClick(element, x, y, duration, getResponseHandler(
...
exports.mobileTap = function(req, res) {
	...
	var tapCount = req.body.tapCount, touchCount = req.body.touchCount, duration = req.body.duration, element = req.body.element, x = req.body.x, y = req.body.y;
...
exports.mobileFlick = function(req, res) {
	...
	var touchCount = req.body.touchCount, element = req.body.element, startX = req.body.startX, startY = req.body.startY, endX = req.body.endX, endY = req.body.endY;
...
exports.mobileDrag = function(req, res) {
	...
	var touchCount = req.body.touchCount, element = req.body.element, destEl = req.body.destEl, duration = req.body.duration, startX = req.body.startX, startY = req.body.startY, endX = req.body.endX, endY = req.body.endY;

	req.device.drag(startX, startY, endX, endY, duration, touchCount, element,
			destEl, getResponseHandler(req, res));
};
...
exports.mobileSwipe = function(req, res, gestures) {
	...
	var touchCount = req.body.touchCount || 1, startX = getCoordDefault(gestures[0].options.x), startY = getCoordDefault(gestures[0].options.y), endX = getCoordDefault(gestures[2].options.x), endY = getCoordDefault(gestures[2].options.y), duration = _getSwipeTouchDuration(gestures[1]), element = gestures[0].options.element, destElement = gestures[2].options.element
			|| gestures[0].options.element;

	// there's no destination element handling in bootstrap and since it applies
	// to all platforms, we handle it here
	if (hasValue(destElement)) {
		req.device
				.getLocation(
						destElement,
						function(err, locResult) {
							if (err) {
										...
										.getSize(
												...
												function(er, sizeResult) {
													if (er) {
														...
														var offsetX = (Math
																.abs(endX) < 1 && Math
																.abs(endX) > 0) ? sizeResult.value.width
																* endX
																: endX;
														var offsetY = (Math
																.abs(endY) < 1 && Math
																.abs(endY) > 0) ? sizeResult.value.height
																* endY
																: endY;

														var destX = locResult.value.x
																+ offsetX;
														var destY = locResult.value.y
																+ offsetY;
...
exports.mobileRotation = function(req, res) {
	...
	var element = req.body.element, duration = req.body.duration, x = req.body.x, y = req.body.y, radius = req.body.radius, touchCount = req.body.touchCount, rotation = req.body.rotation;

	req.device.rotate(x, y, radius, rotation, duration, touchCount, element,
			getResponseHandler(req, res));
};
...
exports.mobilePinchClose = function(req, res) {
	...
	var element = req.body.element, duration = req.body.duration, startX = req.body.startX, startY = req.body.startY, endX = req.body.endX, endY = req.body.endY, percent = req.body.percent, steps = req.body.steps;

	req.device.pinchClose(startX, startY, endX, endY, duration, percent, steps,
			element, getResponseHandler(req, res));
};
...
exports.mobilePinchOpen = function(req, res) {
	...
	req.device.pinchOpen(startX, startY, endX, endY, duration, percent, steps,
			element, getResponseHandler(req, res));
};
...
exports.mobileScrollTo = function(req, res) {
	...
	var element = req.body.element, text = req.body.text, direction = req.body.direction;
...
exports.mobileScroll = function(req, res) {
	...
	var direction = req.body.direction.toString().toLowerCase(), element = req.body.element;
	if (!_.contains([ 'up', 'left', 'right', 'down' ], direction)) {
		return respondError(
				req,
				res,
				status.codes.UnknownCommand.code,
...
exports.hideKeyboard = function(req, res) {
	...
	var strategy = req.body.strategy || (key ? 'pressKey' : 'default');
...
exports.getAttribute = function(req, res) {
	var elementId = req.params.elementId, attributeName = req.params.name;

	req.device.getAttribute(elementId, attributeName, getResponseHandler(req,
			res));
};

exports.getCssProperty = function(req, res) {
	var elementId = req.params.elementId, propertyName = req.params.propertyName;

	req.device.getCssProperty(elementId, propertyName, getResponseHandler(req,
			res));
};
...
exports.getLocation = function(req, res) {
	logCustomDeprecationWarning(
			'location',
			'getLocation',
			"location will be removed in a future version of Appium, please use location_in_view");
...
exports.pressKeyCode = function(req, res) {
	...
	var keycode = req.body.keycode, metastate = req.body.metastate;
...
exports.longPressKeyCode = function(req, res) {
	...
	var keycode = req.body.keycode, metastate = req.body.metastate;
	req.device.longPressKeyCode(keycode, metastate,
			getResponseHandler(req, res));
};
...
exports.timeouts = function(req, res) {
	var timeoutType = req.body.type, ms = req.body.ms;
	if (checkMissingParams(req, res, {
		type : timeoutType,
		ms : ms
	})) {
		if (timeoutType === "implicit") {
			...
			respondError(req, res, status.codes.UnknownCommand.code, new Error(
					"Invalid timeout '" + timeoutType + "'"));
		}
	}
};
...
exports.setLocation = function(req, res) {
	...
	var latitude = location.latitude || req.body.latitude, longitude = location.longitude
			|| req.body.longitude, altitude = location.altitude
			|| req.body.altitude || null;
	req.device.setLocation(latitude, longitude, altitude, null, null, null,
			null, getResponseHandler(req, res));
};
...
exports.moveTo = function(req, res) {
	...
	var xoffset = req.body.xoffset, yoffset = req.body.yoffset, element = req.body.element;
...
exports.pickAFlickMethod = function(req, res) {
	if (typeof req.body.xSpeed !== "undefined"
			|| typeof req.body.xspeed !== "undefined") {
...
exports.flick = function(req, res) {
	var swipe = req.body.swipe, xSpeed = req.body.xSpeed, ySpeed = req.body.ySpeed, element = req.body.element;
	...
	if (checkMissingParams(req, res, {
		...
		if (element) {
			...
			req.device.fakeFlick(xSpeed, ySpeed, swipe, getResponseHandler(req,
					res));
		}
	}
};
...
exports.flickElement = function(req, res) {
	var element = req.body.element, xoffset = req.body.xoffset, yoffset = req.body.yoffset, speed = req.body.speed;
	...
	if (checkMissingParams(req, res, {
		...
		req.device.fakeFlickElement(element, xoffset, yoffset, speed,
				getResponseHandler(req, res));
	}
};

exports.execute = function(req, res) {
	var script = req.body.script, args = req.body.args;
...
exports.executeAsync = function(req, res) {
	var script = req.body.script, args = req.body.args, responseUrl = '';

	responseUrl += 'http://' + req.appium.args.callbackAddress + ':'
			+ req.appium.args.callbackPort;
	responseUrl += '/wd/hub/session/' + req.appium.sessionId
	...
	if (checkMissingParams(req, res, {
		...
		req.device.executeAsync(script, args, responseUrl, getResponseHandler(
				req, res));
	}
};

exports.executeMobileMethod = function(req, res, cmd) {
	var args = req.body.args, params = {};
	...
	if (suppMethods && !_.contains(suppMethods, cmd)) {
		return respondError(req, res, status.codes.UnknownCommand.code,
				new Error(
						"That device doesn't know how to respond to 'mobile: '"
								+ cmd
								+ "--it's probably not using Appium's API"));
	}
			...
			safely(
					...
					function() {
						res
								.status(400)
								.send(
										"Mobile methods only take one parameter, which is a "
...
exports.setValueImmediate = function(req, res) {
	var element = req.params.elementId, value = req.body.value;
	if (checkMissingParams(req, res, {
		element : element,
		value : value
	})) {
		req.device.setValueImmediate(element, value, getResponseHandler(req,
				res));
	}
};
...
exports.getStrings = function(req, res) {
	...
	var language = req.body.language, stringFile = req.body.stringFile;
...
exports.remove = function(req, res) {
	logger.info("Node will be terminated by Grid Hub request.");
	res.sendStatus(200);
	process.exit(1);
};

exports.getTemplate = function(templateName) {
	return swig.compileFile(path.resolve(__dirname, "templates", templateName
			+ ".html"));
};
